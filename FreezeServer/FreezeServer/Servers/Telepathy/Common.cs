...
namespace Telepathy
{
    public abstract class Common
    {
        ...
        public bool GetNextEvent(out StateMessage message)
        {
            return eventQueue.TryDequeue(out message);
        }

        public bool GetNextPackage(out NetworkPackageData package)
        {
            return packageQueue.TryDequeue(out package);
        }
                                          ...
                                          ConcurrentQueue<NetworkPackageData> receivePackage, int MaxMessageSize)
        {
            ...
            try
            {
                ...
                // it's needed
                receiveQueue.Enqueue(new StateMessage(connectionId, EventType.Connected));
                ...
                while (true)
                {
                    ...
                    //networkPackage.connectionID = connectionId;

                    // queue it
                    receivePackage.Enqueue(new NetworkPackageData(connectionId, content));
            ...
            finally
            {
                ...
                //    would be closed.
                receiveQueue.Enqueue(new StateMessage(connectionId, EventType.Disconnected));
            }
        }
